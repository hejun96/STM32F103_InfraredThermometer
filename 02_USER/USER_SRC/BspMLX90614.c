...
void SMBus_Init()
{
	...
	// 温度传感器引脚配置
	GPIO_InitStructure.GPIO_Pin = (SMBUS_SCK|SMBUS_SDA);          //非接触温度传感器SDAL 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;        //管脚频率为50MHZ
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;          //输出模式为
	
	GPIO_Init(GPIOB,&GPIO_InitStructure);                   //初始化PB 10/11
